...
//Hypothetical PhotoGallery Component with integrated changes
const PhotoGallery = ({ category, photos }: { category: string; photos: Photo[] }) => {
  const [selectedPhoto, setSelectedPhoto] = useState<Photo | null>(null);
  const [open, setOpen] = useState(false);

  return (
    <>
      {/* ... rest of the PhotoGallery component ... */}
      <Dialog open={open} onClose={() => setOpen(false)}>
        <DialogContent className="max-w-[95vw] h-[95vh] p-4 flex items-center justify-center">
          {selectedPhoto && (
            <motion.div
            ...
            >
              <img
                ...
                className="max-w-[calc(95vw-2rem)] max-h-[calc(95vh-2rem)] w-auto h-auto object-contain"
                loading="eager"
                decoding="sync"
              />
            </motion.div>
